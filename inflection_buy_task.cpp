...
void InflectionBuyTask::HandleQuoteData()
{
    ...
    if( time_point_open_warning_ != 0 ) // if warning has been opened
    {
        if( iter->cur_price < bottom_price_ )
        {
            ...
            app_->local_logger().LogLocal(utility::FormatStr("%d InflectionBuyTask::HandleQuoteData bottom %.2f", para_.id, bottom_price_));
        }
    ...
    }else if( iter->cur_price > 0 && iter->cur_price < para_.alert_price )
    {
        ...
        bottom_price_ = iter->cur_price; 
        app_->local_logger().LogLocal(utility::FormatStr("%d InflectionBuyTask::HandleQuoteData first open bottom %.2f", para_.id, bottom_price_));
